...
const EixoAcasalamento: React.FC<EixoAcasalamentoProps> = ({ farmId }) => {
  ...
  const plantelSummary = useMemo(() => {
    ...
    const missingWeight = selectedAnimals.filter((animal) => animal.pesoAtual === null || animal.pesoAtual === undefined).length;
    const lightFemales = selectedAnimals.filter((animal) => typeof animal.pesoAtual === 'number' && animal.pesoAtual < 350).length + uploadedLots.filter((lot) => lot.peso_medio < 350).length;
      ...
      <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-5">
          <div className="rounded-[24px] border border-[var(--eixo-border)] bg-[var(--eixo-surface)] p-5">
            ...
            {(targetMode === 'GROUP' || targetMode === 'INDIVIDUAL') && <div className="mt-4 max-h-64 space-y-2 overflow-auto rounded-2xl border border-[var(--eixo-border)] bg-[var(--eixo-surface-soft)] p-3">{animals.map((animal) => { const hasWeight = animal.pesoAtual !== null && animal.pesoAtual !== undefined; const isLight = typeof animal.pesoAtual === 'number' && animal.pesoAtual < 350; return <label key={animal.id} className="flex items-center justify-between gap-3 rounded-xl p-2 text-sm hover:bg-[var(--eixo-surface)]"><span className="flex items-center gap-3"><input type={targetMode === 'INDIVIDUAL' ? 'radio' : 'checkbox'} name="mating-animal" checked={selectedAnimalIds.includes(animal.id)} onChange={(event) => setSelectedAnimalIds((current) => targetMode === 'INDIVIDUAL' ? [animal.id] : event.target.checked ? [...current, animal.id] : current.filter((id) => id !== animal.id))} /><span><strong>{animal.brinco}</strong> · {hasWeight ? `${animal.pesoAtual} kg` : 'sem peso'}</span></span><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${isLight || !hasWeight ? 'bg-[#fbede8] text-[#8c4d39]' : 'bg-[var(--eixo-green-soft)] text-[var(--eixo-success)]'}`}>{isLight ? 'atenção' : hasWeight ? 'apta' : 'sem peso'}</span></label>; })}{!animals.length && <p className="text-sm text-[var(--eixo-text-muted)]">Nenhuma matriz disponível.</p>}</div>}
